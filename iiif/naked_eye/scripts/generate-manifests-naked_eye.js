...
async function generateManifests() {
	try {
		...
		const MANIFEST_BASE_PATH = `${GITHUB_BASE_URL}/iiif/naked_eye/manifests`;
		const IMAGE_BASE_PATH = `${IIIF_BASE_URL}/coins%2Fnaked_eye%2F`;

		// Local path to the folder containing the image files
		// Adjust this path to match your local setup
		const LOCAL_IMAGE_PATH =
			"/Users/fv/Documents/Lohnarbeit/Brogramming/DSD Numismatik/Material/20250320 All data/Complete_Data/Images_final/naked_eye";

		// Define file paths
		const TEMPLATE_PATH = "../manifests/_template-naked_eye.json";
		...
		// Filter to only include coins with image_selection and naked_eye category
		const nakedEyeCoins = data.filter(
			(coin) =>
				coin.DSD_project_info &&
				coin.DSD_project_info.exhibition_category === "naked_eye" &&
				coin.image_selection
		);

		console.log(
			`Found ${nakedEyeCoins.length} coins with naked_eye category and image selection`
		);

		// Keep track of used IDs to avoid duplicates
		const usedIds = new Set();

		for (const [coinIndex, coin] of nakedEyeCoins.entries()) {
			manifestCount++;
			...
			addMetadata("Sammlung", "More than the Naked Eye Can See");
